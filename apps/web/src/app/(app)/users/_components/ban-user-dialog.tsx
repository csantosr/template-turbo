...
export function BanUserDialog({ userId, userName, userEmail }: Props) {
  ...
  return (
    <ActionDialog
      trigger={
        <Button size="icon" variant="ghost" className="text-destructive hover:opacity-70" tooltip="Ban user">
          <Prohibit weight="bold" size={16} />
        </Button>
      }
      title="Ban User"
      description={`${userName} · ${userEmail}`}
      onOpenChange={(open) => { if (!open) { setError(null); setReason(""); setDuration(""); } }}
    >
      {({ close }) => (
        <>
          <div className="flex flex-col gap-4">
            <label className="flex flex-col gap-1">
              <span className="font-mono text-sm uppercase tracking-widest text-muted-foreground">Reason (optional)</span>
              <Input
                value={reason}
            ...
            <label className="flex flex-col gap-1">
              <span className="font-mono text-sm uppercase tracking-widest text-muted-foreground">Duration</span>
              <select
                value={duration}
